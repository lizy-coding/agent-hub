...
def build_release_program(context: dict[str, object], *, version: str, tag: str, name: str, notes: str, draft: bool, prerelease: bool, artifacts: list[dict[str, object]], evidence: list[str]) -> dict[str, object]:
    return {"project_id": context.get("project_id"), "program_id": context.get("program_id"), "adapter": context.get("adapter"), "primary_repository_id": context.get("primary_repository_id", "flutter_forge"), "cluster_root": context.get("cluster_root"), "status": "READY", "execution_mode": "PLAN_ONLY", "evidence": evidence, "build_matrix": dict((context.get("release") or {}).get("build_matrix") or {}), "release": {"version": version, "tag": tag, "name": name, "notes": notes, "draft": draft, "prerelease": prerelease, "artifacts": artifacts}}

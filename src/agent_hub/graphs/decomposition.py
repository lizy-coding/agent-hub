...
def _repositories_for(task: dict[str, object]) -> list[str]:
    ...
    for unit in [*task.get("source_units", []), *task.get("target_units", [])]:
        text = str(unit)
        if text.startswith("flutter_study/") or text.startswith("apps/flutter_study"):
    ...
    def reconcile(state: State):
        ...
        if isinstance(decision, dict) and decision.get("choice") == "CREATE_APPS_FLUTTER_STUDY":
            ...
            if task_id == "relocate-flutter-study-app" and isinstance(blocker, dict) and blocker.get("task_id") == task_id and isinstance(task, dict):
                ...
                return {"decomposition_program": program, "worker_result": {}, "integration_result": {"status": "APP_RELOCATION_CONTRACT_AUTHORIZED", "task_id": task_id}}
        if isinstance(decision, dict) and decision.get("choice") == "retry":
            ...
            if isinstance(blocker, dict) and isinstance(task, dict) and blocker.get("task_id") == task_id and blocker.get("status") in retryable:
                ...
                return {"decomposition_program": program, "worker_result": {}, "integration_result": {"status": "RETRY_AUTHORIZED", "task_id": task_id}}

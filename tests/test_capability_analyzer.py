...
class CapabilityAnalyzerTest(unittest.TestCase):
 @classmethod
 def setUpClass(cls): cls.analyzer=CapabilityAnalyzer(WorkspaceConfig.from_file(Path(__file__).parents[1]/'workspace/config.json'))
 def test_context_consumption_evidence_and_unknowns(self):
  analysis=self.analyzer.analyze_capabilities('GCode import callsite and plugin adapter','flutter_study')
  self.assertTrue(analysis.capabilities); self.assertTrue(analysis.unknowns)
  self.assertTrue(all(node.evidence_refs for node in analysis.capabilities)); self.assertEqual(self.analyzer.validate_capability_analysis(analysis),[])
 def test_ownership_coupling_and_structured_extraction(self):
  ...
  analysis=self.analyzer.analyze_capabilities('riverpod platform controller widget parser','flutter_study')
